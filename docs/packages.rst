...
There ``DEBUG`` value is use to distinguish between development and production environments.
...
.. code-block:: shell
   ...
   just run myjourney # Prints all available django commands
   just run myjourney db_worker # Runs the django-tasks database worker for background tasks
   just run myjourney setup # Runs the custom setup command
   just run myjourney runserver # Runs the django dev server
   just run myjourney dbshell # Opens the dbshell
