(() => {
  const initSelect = (selectComponent) => {
    ...
    let options = allOptions.filter(opt => opt.getAttribute('aria-disabled') !== 'true');
    ...
    const refreshOptions = () => {
      ...
      options = allOptions.filter(opt => opt.getAttribute('aria-disabled') !== 'true');
      visibleOptions = options.filter(opt => opt.getAttribute('aria-hidden') !== 'true');
      setActiveOption(-1);
    };
    ...
    if (isMultiple) {
      ...
      const initialOption = options.find(opt => getValue(opt) === input.value && !opt.hasAttribute('data-create'))
        || options.find(opt => !opt.hasAttribute('data-create'));
    ...
    selectComponent.addEventListener('basecoat:options-changed', refreshOptions);
